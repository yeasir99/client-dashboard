...
if(params.id){
    ...
    const handleCencel = async () =>{
        const res = await axios.post('https://kblsf.site/DLogicKBL/salesforce_api.php?action=create_sndApprovalRejected_CancelledVisitPlan', {
        ...
        router.push('/dashboard/visit-approval')
    }
    const handleApprove = async () =>{
        const res = await axios.post(`https://kblsf.site/DLogicKBL/salesforce_api.php?action=create_sndApprovalDetailsVisitPlan&VisitPlanID=${params.id}`, {
            VisitPlanID: prevData.data.VisitPlan.VisitPlanID,
...
<div className="flex justify-between py-3 px-1">
  <button className="bg-red-400 px-6 py-2 rounded-md text-white" onClick={handleCencel}>
    Cencel
  </button>
  <button className="bg-primary px-6 py-2 rounded-md text-white" onClick={handleApprove}>
    Approval
  </button>
</div>
</div>
