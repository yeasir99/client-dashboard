...
const SpecimanPendingList = () => {
    const {status, data} = useGetData('https://kblsf.site/DLogicKBL/salesforce_api.php?action=get_specimenordersInvoice')
    if(status === 'pending'){
                    ...
                    {data.length > 0 && data.map(item => (
                      ...
                      >
                        ...
                        <td className="whitespace-nowrap px-6 py-4 flex justify-center items-center gap-3">
                          <Link href={`/dashboard/invoice-bill/add/speciman/${item.SalesOrderID}`}>
